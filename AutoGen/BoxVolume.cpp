...
std::vector <
  KittingWorkstation * >*BoxVolume::gethasWorkstation_OtherObstacles ()
{
...
void
BoxVolume::setdao (DAO * _dao)
{
  this->dao = _dao;
}

void
BoxVolume::sethasBoxVolume_MaximumPoint (Point * _hasBoxVolume_MaximumPoint)
{
  this->hasBoxVolume_MaximumPoint = _hasBoxVolume_MaximumPoint;
}

void
BoxVolume::sethasBoxVolume_MinimumPoint (Point * _hasBoxVolume_MinimumPoint)
{
  this->hasBoxVolume_MinimumPoint = _hasBoxVolume_MinimumPoint;
}

void
BoxVolume::sethasWorkstation_OtherObstacles (std::vector <
					     KittingWorkstation *
...
void
BoxVolume::sethasRobot_WorkVolume (std::vector <
				   Robot * >_hasRobot_WorkVolume)
{
  this->hasRobot_WorkVolume = _hasRobot_WorkVolume;
}

void
BoxVolume::get (std::string name)
{
...
void
BoxVolume::copy (std::map < std::string, std::string > object)
{
  ...
  if (this->hasBoxVolume_MaximumPoint == NULL
    ...
    {
      this->hasBoxVolume_MaximumPoint =
	new Point (object["hasBoxVolume_MaximumPoint/Point._NAME"]);
    }
  if (this->hasBoxVolume_MinimumPoint == NULL
      && object["hasBoxVolume_MinimumPoint/Point._NAME"] != "")
    {
      this->hasBoxVolume_MinimumPoint =
	new Point (object["hasBoxVolume_MinimumPoint/Point._NAME"]);
    }
  if (this->hasWorkstation_OtherObstacles.empty ()
      ...
      "")
    {
      temp =
	Explode (object
		 ["hasWorkstation_OtherObstacles/KittingWorkstation._NAME"],
		 ' ');
      for (unsigned int i = 0; i < temp.size (); i++)
	{
	  this->hasWorkstation_OtherObstacles.
	    push_back (new KittingWorkstation (temp[i]));
	}
    }
