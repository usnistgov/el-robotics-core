...
std::map < std::string,
  std::vector < std::string > >DAO::getSqlQueriesDataMulti;
std::map < std::string,
  std::vector < std::string > >DAO::getSqlQueriesObjectSingle;
std::map < std::string,
  std::vector < std::string > >DAO::getSqlQueriesObjectMulti;

void
DAO::fillGetSqlQueries ()
{
  ...
  temp.clear ();
  temp.
    push_back ("LargeContainer/hasLargeBoxWithEmptyKitTrays_LargeContainer");
  ...
  temp.clear ();
  temp.
    push_back ("-EndEffectorHolder/hadByEndEffectorHolder_ChangingStation");
  ...
  temp.
    push_back ("EndEffectorChangingStation/hasWorkstation_ChangingStation");
...
void
DAO::setclassName (std::vector < std::string > _className)
{
  this->className = _className;
}

void
DAO::setconnection (Connection * _connection)
{
...
std::map < std::string, std::string > DAO::get (std::string name)
{
  ...
  try
  {
    ...
    prep_stmt =
      connection->getCon ()->
      prepareStatement (DAO::getSqlQueriesDataSingle[className.back ()]);
	...
	try
	{
	  prep_stmt =
	    connection->getCon ()->
	    prepareStatement (DAO::
			      getSqlQueriesDataMulti[className.back ()][i]);
	  prep_stmt->setInt (1, id);
	  res = prep_stmt->executeQuery ();
	  while (res->next ())
	    {
	...
	try
	{
	  ...
	  if (getSqlQueriesObjectSingle[className.back ()][i].substr (0, 1)
		...
		= stmt->executeQuery ("SELECT "
				      +
				      getSqlQueriesObjectSingle[className.
								back ()][i].
				      substr (0,
				      ...
				      getSqlQueriesObjectSingle[className.
								back ()][i].
				      substr (getSqlQueriesObjectSingle
					      [className.back ()][i].
					      find ("/") + 1) + " = " +
				      getSqlQueriesObjectSingle[className.
								back ()][i].
	  ...
	  do
	      ...
	      do
		{
		  ...
		  for (int j = 0; j < (int) res_meta->getColumnCount (); ++j)
		    {
		      if (res->isFirst ())
			map[getSqlQueriesObjectSingle[className.back ()][i].
			    substr (getSqlQueriesObjectSingle
				    [className.back ()][i].find ("/") + 1) +
		  ...
		  if (getSqlQueriesObjectSingle[className.back ()][i].
		      substr (0, 1) != "-")
		    {
		      newName
			=
			res->
			getString (getSqlQueriesObjectSingle
				   [className.back ()][i].substr (0,
								  getSqlQueriesObjectSingle
								  [className.
								   back ()]
		  ...
		  if (getSqlQueriesObjectSingle[className.back ()][i].
		      substr (0, 1) != "-")
		    {
		      newTable
			=
			getSqlQueriesObjectSingle[className.back ()][i].
			substr (0,
				getSqlQueriesObjectSingle[className.
							  back ()][i].
				find ("/"));
	...
	try
	{
	  ...
	  restempmulti
	    = stmtmulti->executeQuery ("SELECT "
				       +
				       getSqlQueriesObjectMulti[className.
								back ()][m].
				       substr (0,
				       ...
				       getSqlQueriesObjectMulti[className.
								back ()][m].
				       substr (getSqlQueriesObjectMulti
					       [className.back ()][m].
					       find ("/") + 1) + " WHERE " +
	  ...
	  do
		...
		prepareStatement (DAO::
				  getSqlQueriesDataSingle
				  [getSqlQueriesObjectMulti[className.back ()]
				   [m].substr (0,
					       getSqlQueriesObjectMulti
					       [className.back ()][m].
					       find ("/"))]);
	      ...
	      for (int i = 0; i < (int) res_meta->getColumnCount (); ++i)
		{
		  map[getSqlQueriesObjectMulti[className.back ()][m].
		      substr (getSqlQueriesObjectMulti[className.back ()][m].
			      find ("/") + 1) + "/" +
	      ...
	      for (int unsigned i = 0; i
		   <
		   DAO::
		   getSqlQueriesDataMulti[getSqlQueriesObjectMulti
					  [className.back ()][m].substr (0,
									 getSqlQueriesObjectMulti
									 [className.
									  back
									  ()]
		   ...
		   size (); i++)
		{
		  try
		  {
		    ...
		    res_meta = res->getMetaData ();
		    map[getSqlQueriesObjectMulti[className.back ()][m].
			substr (getSqlQueriesObjectMulti[className.back ()]
				[m].find ("/") + 1) + "/" +
		   ...
		   < DAO::getSqlQueriesObjectSingle[table].size (); i++)
		{
		  try
		  {
			...
			prep_stmt =
			  ...
			  prepareStatement (DAO::
					    getSqlQueriesDataSingle
					    [getSqlQueriesObjectSingle[table]
					     [i].substr (0,
							 getSqlQueriesObjectSingle
							 [table][i].
							 find ("/"))]);
			...
			do
			  {
			    ...
			    for (int j = 0; j
				 < (int) res_meta->getColumnCount (); ++j)
			      {
				if (res->isFirst ())
				  map[getSqlQueriesObjectSingle[table][i].
				      substr (getSqlQueriesObjectSingle[table]
					      [i].find ("/") + 1) + "/" +
				      res_meta->getColumnLabel (j + 1)] =
				    res->getString (j + 1);
				else
			    ...
			    if (getSqlQueriesObjectSingle[table][i].
				substr (0, 1) != "-")
			      {
				newName
				  =
				  res->
				  getString (getSqlQueriesObjectSingle[table]
					     [i].substr (0,
							 getSqlQueriesObjectSingle
							 [table][i].
							 find ("/")) +
				...
				newTable
				  =
				  getSqlQueriesObjectSingle[table][i].
				  substr (0,
					  getSqlQueriesObjectSingle[table][i].
					  find ("/"));
			      }
...
void
DAO::set (std::map < std::string, std::string > data)
{
  try
  {
    // DATA SINGLE
    ...
    for (std::map < std::string, std::string >::iterator it = data.begin ();
	 it != data.end (); it++)
      {
	if (DAO::getSqlQueriesDataSingle[this->className.back ()].
	    find (this->className.back () + "." + it->first) !=
	    std::string::npos)
	  {
	    int val;
  ...
  for (int unsigned i = 0; i
	...
	query
	  ...
	  substr (DAO::getSqlQueriesDataMulti[className.back ()][i].
		  find ("FROM") + 5);
	query = query.substr (0, query.length () - 2) + " ";	//to delete the last ?
	...
	if (DAO::getSqlQueriesObjectSingle[className.back ()][i].substr (0, 1)
	    != "-")
	  {
	    ...
	    query
	      = query + "SET "
	      +
	      DAO::getSqlQueriesObjectSingle[className.back ()][i].
	      substr (DAO::getSqlQueriesObjectSingle[className.back ()][i].
		      find ("/") + 1) + "='" +
	      data[DAO::getSqlQueriesObjectSingle[className.back ()][i].
		   substr (DAO::
			   getSqlQueriesObjectSingle[className.back ()][i].
			   find ("/") + 1)];
	    query =
	      query + "' WHERE " + this->className.back () + "ID=" +
	      data[this->className.back () + "ID"];
	    if (!data
		[DAO::getSqlQueriesObjectSingle[className.back ()][i].
		 substr (DAO::getSqlQueriesObjectSingle[className.back ()][i].
	...
	else
	  {
	    query
	      = "UPDATE "
	      +
	      DAO::getSqlQueriesObjectSingle[className.back ()][i].substr (1,
									   DAO::
									   getSqlQueriesObjectSingle
									   [className.
									    back
  ...
  for (int unsigned i = 0; i
	...
	query
	  = query
	  +
	  DAO::getSqlQueriesObjectMulti[className.back ()][i].
	  substr (DAO::getSqlQueriesObjectMulti[className.back ()][i].
		  find ("/") + 1);
	query =
	  query + " WHERE " + this->className.back () + "ID=" +
	  data[this->className.back () + "ID"];
	...
	std::vector < std::string >
	  ...
	  Explode (data
		   [DAO::getSqlQueriesObjectMulti[className.back ()][i].
		    substr (DAO::
			    getSqlQueriesObjectMulti[className.back ()][i].
			    find ("/") + 1)], ' ');
	if (!data
	    [DAO::getSqlQueriesObjectMulti[className.back ()][i].
	     substr (DAO::getSqlQueriesObjectMulti[className.back ()][i].
	  ...
	  for (int unsigned i = 0; i < multi.size (); i++)
		...
		substr (DAO::getSqlQueriesObjectMulti[className.back ()][i].
	      ...
	      query =
		query + " (" + this->className.back () + "ID" + "," +
		DAO::getSqlQueriesObjectMulti[className.back ()][i].substr (0,
									    DAO::
									    getSqlQueriesObjectMulti
									    [className.
									     back
...
std::vector < std::string > DAO::Explode (const std::string & str,
					  char separator)
{
