...
KittingWorkstation *
KitDesign::gethadByKitDesign_Workstation ()
{
  return hadByKitDesign_Workstation;
}

std::vector <
  PartRefAndPose * >*KitDesign::gethadByPartRefAndPose_KitDesign ()
{
  return &hadByPartRefAndPose_KitDesign;
}

void
KitDesign::sethasKitDesign_Id (std::string _hasKitDesign_Id)
{
  this->hasKitDesign_Id = _hasKitDesign_Id;
}

void
KitDesign::sethasKitDesign_KitTraySkuRef (std::
					  string _hasKitDesign_KitTraySkuRef)
{
  this->hasKitDesign_KitTraySkuRef = _hasKitDesign_KitTraySkuRef;
}

void
KitDesign::setdao (DAO * _dao)
{
  this->dao = _dao;
}

void
KitDesign::sethadByKitDesign_Workstation (KittingWorkstation *
...
void
KitDesign::sethadByPartRefAndPose_KitDesign (std::vector <
					     PartRefAndPose *
...
KitDesign::set (std::string name)
{
  ...
  if (hadByKitDesign_Workstation != NULL)
    data["hadByKitDesign_Workstation"] =
      hadByKitDesign_Workstation->getname ();
  for (unsigned int i = 0; i < hadByPartRefAndPose_KitDesign.size (); ++i)
    {
      ss.str ("");
      hadByPartRefAndPose_KitDesign[i]->
	get (hadByPartRefAndPose_KitDesign[i]->getname ());
      ss << hadByPartRefAndPose_KitDesign[i]->getPartRefAndPoseID ();
      data["hadByPartRefAndPose_KitDesign"] =
	data["hadByPartRefAndPose_KitDesign"] + " " + ss.str ();
    }
  dao = new DAO ("KitDesign");
  dao->set (data);
  delete (dao);
...
void
KitDesign::copy (std::map < std::string, std::string > object)
{
  ...
  this->hasKitDesign_KitTraySkuRef =
    object["KitDesign.hasKitDesign_KitTraySkuRef"];
  this->name = object["KitDesign._NAME"];
  this->KitDesignID = std::atof (object["KitDesign.KitDesignID"].c_str ());
  if (this->hadByKitDesign_Workstation == NULL
      && object["hadByKitDesign_Workstation/KittingWorkstation._NAME"] != "")
    {
      this->hadByKitDesign_Workstation =
	new
	KittingWorkstation (object
			    ["hadByKitDesign_Workstation/KittingWorkstation._NAME"]);
    }
  if (this->hadByPartRefAndPose_KitDesign.empty ()
      && object["hadByPartRefAndPose_KitDesign/PartRefAndPose._NAME"] != "")
    {
      temp =
	Explode (object["hadByPartRefAndPose_KitDesign/PartRefAndPose._NAME"],
		 ' ');
...
} std::vector < std::string > KitDesign::Explode (const std::string & str,
						  char separator)
{
