...
int
KittingWorkstation::getKittingWorkstationID ()
{
  return KittingWorkstationID;
}

DAO *
KittingWorkstation::getdao ()
{
  return dao;
}

std::vector <
  KitDesign * >*KittingWorkstation::gethadByKitDesign_Workstation ()
{
...
std::vector <
  StockKeepingUnit * >*KittingWorkstation::gethadBySku_Workstation ()
{
...
void
KittingWorkstation::sethasWorkstation_LengthUnit (std::
						  string
						  _hasWorkstation_LengthUnit)
{
  this->hasWorkstation_LengthUnit = _hasWorkstation_LengthUnit;
}

void
KittingWorkstation::sethasWorkstation_AngleUnit (std::
						 string
						 _hasWorkstation_AngleUnit)
{
  this->hasWorkstation_AngleUnit = _hasWorkstation_AngleUnit;
}

void
KittingWorkstation::sethasWorkstation_WeightUnit (std::
						  string
						  _hasWorkstation_WeightUnit)
{
  this->hasWorkstation_WeightUnit = _hasWorkstation_WeightUnit;
}

void
KittingWorkstation::setdao (DAO * _dao)
{
  this->dao = _dao;
}

void
KittingWorkstation::sethadByKitDesign_Workstation (std::vector <
						   KitDesign *
						   >_hadByKitDesign_Workstation)
{
...
void
KittingWorkstation::
sethasWorkstation_ChangingStation (EndEffectorChangingStation *
				   _hasWorkstation_ChangingStation)
{
  this->hasWorkstation_ChangingStation = _hasWorkstation_ChangingStation;
}

void
KittingWorkstation::sethasWorkstation_WorkTable (WorkTable *
...
void
KittingWorkstation::sethasWorkstation_OtherObstacles (std::vector <
						      BoxVolume *
...
void
KittingWorkstation::sethadBySku_Workstation (std::vector <
					     StockKeepingUnit *
					     >_hadBySku_Workstation)
{
  this->hadBySku_Workstation = _hadBySku_Workstation;
}

void
KittingWorkstation::sethasWorkstation_Robot (Robot * _hasWorkstation_Robot)
{
  this->hasWorkstation_Robot = _hasWorkstation_Robot;
}

void
KittingWorkstation::get (std::string name)
{
...
void
KittingWorkstation::set (std::string name)
{
  ...
  for (unsigned int i = 0; i < hadByKitDesign_Workstation.size (); ++i)
    {
      ss.str ("");
      hadByKitDesign_Workstation[i]->get (hadByKitDesign_Workstation[i]->
					  getname ());
      ss << hadByKitDesign_Workstation[i]->getKitDesignID ();
      data["hadByKitDesign_Workstation"] =
	data["hadByKitDesign_Workstation"] + " " + ss.str ();
    }
  if (hasWorkstation_ChangingStation != NULL)
    data["hasWorkstation_ChangingStation"] =
      hasWorkstation_ChangingStation->getname ();
  ...
  for (unsigned int i = 0; i < hasWorkstation_OtherObstacles.size (); ++i)
    {
      ss.str ("");
      hasWorkstation_OtherObstacles[i]->
	get (hasWorkstation_OtherObstacles[i]->getname ());
  ...
  for (unsigned int i = 0; i < hadBySku_Workstation.size (); ++i)
    {
      ...
      data["hadBySku_Workstation"] =
	data["hadBySku_Workstation"] + " " + ss.str ();
    }
...
void
KittingWorkstation::copy (std::map < std::string, std::string > object)
{
  ...
  this->hasWorkstation_LengthUnit =
    object["KittingWorkstation.hasWorkstation_LengthUnit"];
  this->hasWorkstation_AngleUnit =
    object["KittingWorkstation.hasWorkstation_AngleUnit"];
  this->hasWorkstation_WeightUnit =
    object["KittingWorkstation.hasWorkstation_WeightUnit"];
  this->name = object["KittingWorkstation._NAME"];
  this->KittingWorkstationID =
    std::atof (object["KittingWorkstation.KittingWorkstationID"].c_str ());
  if (this->hadByKitDesign_Workstation.empty ()
      && object["hadByKitDesign_Workstation/KitDesign._NAME"] != "")
    {
      temp =
	Explode (object["hadByKitDesign_Workstation/KitDesign._NAME"], ' ');
      for (unsigned int i = 0; i < temp.size (); i++)
	{
	  this->hadByKitDesign_Workstation.
	    push_back (new KitDesign (temp[i]));
	}
    }
  if (this->hasWorkstation_ChangingStation == NULL
      &&
      object
      ["hasWorkstation_ChangingStation/EndEffectorChangingStation._NAME"] !=
      "")
    {
      this->hasWorkstation_ChangingStation =
	new
	EndEffectorChangingStation (object
				    ["hasWorkstation_ChangingStation/EndEffectorChangingStation._NAME"]);
    }
  if (this->hasWorkstation_WorkTable == NULL
      && object["hasWorkstation_WorkTable/WorkTable._NAME"] != "")
    {
      this->hasWorkstation_WorkTable =
	new WorkTable (object["hasWorkstation_WorkTable/WorkTable._NAME"]);
    }
  if (this->hasWorkstation_OtherObstacles.empty ()
      && object["hasWorkstation_OtherObstacles/BoxVolume._NAME"] != "")
    {
      temp =
	Explode (object["hasWorkstation_OtherObstacles/BoxVolume._NAME"],
		 ' ');
      for (unsigned int i = 0; i < temp.size (); i++)
	{
	  this->hasWorkstation_OtherObstacles.
	    push_back (new BoxVolume (temp[i]));
	}
    }
  if (this->hadBySku_Workstation.empty ()
      && object["hadBySku_Workstation/StockKeepingUnit._NAME"] != "")
    {
      temp =
	Explode (object["hadBySku_Workstation/StockKeepingUnit._NAME"], ' ');
      for (unsigned int i = 0; i < temp.size (); i++)
	{
	  this->hadBySku_Workstation.
	    push_back (new StockKeepingUnit (temp[i]));
	}
    }
  if (this->hasWorkstation_Robot == NULL
      && object["hasWorkstation_Robot/Robot._NAME"] != "")
    {
      this->hasWorkstation_Robot =
	new Robot (object["hasWorkstation_Robot/Robot._NAME"]);
    }

}

std::vector < std::string >
  KittingWorkstation::Explode (const std::string & str, char separator)
{
