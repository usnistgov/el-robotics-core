...
int
PhysicalLocation::getPhysicalLocationID ()
{
  return PhysicalLocationID;
}

DAO *
PhysicalLocation::getdao ()
{
  return dao;
}
...
std::vector <
  SolidObject * >*PhysicalLocation::gethasSolidObject_SecondaryLocation ()
{
...
void
PhysicalLocation::setdao (DAO * _dao)
{
  this->dao = _dao;
}

void
PhysicalLocation::sethasPhysicalLocation_RefObject (SolidObject *
...
void
PhysicalLocation::sethasSolidObject_SecondaryLocation (std::vector <
						       SolidObject *
...
void
PhysicalLocation::sethasSolidObject_PrimaryLocation (std::vector <
						     SolidObject *
...
void
PhysicalLocation::get (std::string name)
{
...
void
PhysicalLocation::set (std::string name)
{
  ...
  for (unsigned int i = 0; i < hasSolidObject_PrimaryLocation.size (); ++i)
    {
      ss.str ("");
      hasSolidObject_PrimaryLocation[i]->
	get (hasSolidObject_PrimaryLocation[i]->getname ());
      ss << hasSolidObject_PrimaryLocation[i]->getSolidObjectID ();
      data["hasSolidObject_PrimaryLocation"] =
	data["hasSolidObject_PrimaryLocation"] + " " + ss.str ();
    }
  dao = new DAO ("PhysicalLocation");
  dao->set (data);
  delete (dao);
...
void
PhysicalLocation::copy (std::map < std::string, std::string > object)
{
  ...
  this->PhysicalLocationID =
    std::atof (object["PhysicalLocation.PhysicalLocationID"].c_str ());
  if (this->hasPhysicalLocation_RefObject == NULL
	...
	new
	SolidObject (object
		     ["hasPhysicalLocation_RefObject/SolidObject._NAME"]);
    }
  if (this->hasSolidObject_SecondaryLocation.empty ()
      && object["hasSolidObject_SecondaryLocation/SolidObject._NAME"] != "")
    {
      temp =
	Explode (object["hasSolidObject_SecondaryLocation/SolidObject._NAME"],
		 ' ');
  ...
  if (this->hasSolidObject_PrimaryLocation.empty ()
      && object["hasSolidObject_PrimaryLocation/SolidObject._NAME"] != "")
    {
      temp =
	Explode (object["hasSolidObject_PrimaryLocation/SolidObject._NAME"],
		 ' ');
      for (unsigned int i = 0; i < temp.size (); i++)
	{
  ...
  PhysicalLocation::Explode (const std::string & str, char separator)
{
