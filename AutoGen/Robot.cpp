...
double
Robot::gethasRobot_MaximumLoadWeight ()
{
...
void
Robot::sethasRobot_Description (std::string _hasRobot_Description)
{
  this->hasRobot_Description = _hasRobot_Description;
}

void
Robot::sethasRobot_Id (std::string _hasRobot_Id)
{
  this->hasRobot_Id = _hasRobot_Id;
}

void
Robot::sethasRobot_MaximumLoadWeight (double _hasRobot_MaximumLoadWeight)
{
  this->hasRobot_MaximumLoadWeight = _hasRobot_MaximumLoadWeight;
}

void
Robot::setdao (DAO * _dao)
{
  this->dao = _dao;
}

void
Robot::sethadByEndEffector_Robot (EndEffector * _hadByEndEffector_Robot)
{
  this->hadByEndEffector_Robot = _hadByEndEffector_Robot;
}

void
Robot::sethasRobot_WorkVolume (std::vector <
			       BoxVolume * >_hasRobot_WorkVolume)
{
  this->hasRobot_WorkVolume = _hasRobot_WorkVolume;
}

void
Robot::sethasWorkstation_Robot (KittingWorkstation * _hasWorkstation_Robot)
{
  this->hasWorkstation_Robot = _hasWorkstation_Robot;
}

void
Robot::get (std::string name)
{
  ...
  for (unsigned int i = 0; i < hasRobot_WorkVolume.size (); ++i)
    {
      ...
      data["hasRobot_WorkVolume"] =
	data["hasRobot_WorkVolume"] + " " + ss.str ();
    }
...
void
Robot::copy (std::map < std::string, std::string > object)
{
  ...
  this->hasRobot_MaximumLoadWeight =
    std::atof (object["Robot.hasRobot_MaximumLoadWeight"].c_str ());
  this->name = object["Robot._NAME"];
  this->RobotID = std::atof (object["Robot.RobotID"].c_str ());
  if (this->hadByEndEffector_Robot == NULL
      && object["hadByEndEffector_Robot/EndEffector._NAME"] != "")
    {
      this->hadByEndEffector_Robot =
	new EndEffector (object["hadByEndEffector_Robot/EndEffector._NAME"]);
    }
  if (this->hasRobot_WorkVolume.empty ()
      && object["hasRobot_WorkVolume/BoxVolume._NAME"] != "")
    {
  ...
  if (this->hasWorkstation_Robot == NULL
      && object["hasWorkstation_Robot/KittingWorkstation._NAME"] != "")
    {
      this->hasWorkstation_Robot =
	new
	KittingWorkstation (object
			    ["hasWorkstation_Robot/KittingWorkstation._NAME"]);
    }

}

std::vector < std::string > Robot::Explode (const std::string & str,
					    char separator)
{
