...
double
StockKeepingUnit::gethasSku_Weight ()
{
...
int
StockKeepingUnit::getStockKeepingUnitID ()
{
  return StockKeepingUnitID;
}

DAO *
StockKeepingUnit::getdao ()
{
  return dao;
}

KittingWorkstation *
StockKeepingUnit::gethadBySku_Workstation ()
{
...
void
StockKeepingUnit::sethasSku_EndEffectorRefs (std::vector < std::string >
					     _hasSku_EndEffectorRefs)
{
  this->hasSku_EndEffectorRefs = _hasSku_EndEffectorRefs;
}

void
StockKeepingUnit::sethasSku_Description (std::string _hasSku_Description)
{
  this->hasSku_Description = _hasSku_Description;
}

void
StockKeepingUnit::sethasSku_Id (std::string _hasSku_Id)
{
  this->hasSku_Id = _hasSku_Id;
}

void
StockKeepingUnit::sethasSku_Weight (double _hasSku_Weight)
{
  this->hasSku_Weight = _hasSku_Weight;
}

void
StockKeepingUnit::setdao (DAO * _dao)
{
  this->dao = _dao;
}

void
StockKeepingUnit::sethadBySku_Workstation (KittingWorkstation *
...
void
StockKeepingUnit::sethasSku_Shape (ShapeDesign * _hasSku_Shape)
{
  this->hasSku_Shape = _hasSku_Shape;
}

void
StockKeepingUnit::get (std::string name)
{
...
void
StockKeepingUnit::set (std::string name)
{
  ...
  for (unsigned int i = 0; i < hasSku_EndEffectorRefs.size (); ++i)
    {
      data["hasSku_EndEffectorRefs"] =
	data["hasSku_EndEffectorRefs"] + " " + hasSku_EndEffectorRefs[i];
    }
...
void
StockKeepingUnit::copy (std::map < std::string, std::string > object)
{
  ...
  this->hasSku_Weight =
    std::atof (object["StockKeepingUnit.hasSku_Weight"].c_str ());
  this->name = object["StockKeepingUnit._NAME"];
  this->StockKeepingUnitID =
    std::atof (object["StockKeepingUnit.StockKeepingUnitID"].c_str ());
  if (this->hadBySku_Workstation == NULL
      && object["hadBySku_Workstation/KittingWorkstation._NAME"] != "")
    {
      this->hadBySku_Workstation =
	new
	KittingWorkstation (object
			    ["hadBySku_Workstation/KittingWorkstation._NAME"]);
    }
  if (this->hasSku_Shape == NULL
      && object["hasSku_Shape/ShapeDesign._NAME"] != "")
    {
      this->hasSku_Shape =
	new ShapeDesign (object["hasSku_Shape/ShapeDesign._NAME"]);
    }

}

std::vector < std::string >
  StockKeepingUnit::Explode (const std::string & str, char separator)
{
