...
void
Vector::sethasVector_K (double _hasVector_K)
{
  this->hasVector_K = _hasVector_K;
}

void
Vector::sethasVector_J (double _hasVector_J)
{
  this->hasVector_J = _hasVector_J;
}

void
Vector::sethasVector_I (double _hasVector_I)
{
  this->hasVector_I = _hasVector_I;
}

void
Vector::setdao (DAO * _dao)
{
  this->dao = _dao;
}

void
Vector::sethasPartRefAndPose_ZAxis (std::vector <
				    PartRefAndPose *
				    >_hasPartRefAndPose_ZAxis)
{
  this->hasPartRefAndPose_ZAxis = _hasPartRefAndPose_ZAxis;
}

void
Vector::sethasPartRefAndPose_XAxis (std::vector <
				    PartRefAndPose *
				    >_hasPartRefAndPose_XAxis)
{
  this->hasPartRefAndPose_XAxis = _hasPartRefAndPose_XAxis;
}

void
Vector::sethasPoseLocation_ZAxis (std::vector <
				  PoseLocation * >_hasPoseLocation_ZAxis)
{
  this->hasPoseLocation_ZAxis = _hasPoseLocation_ZAxis;
}

void
Vector::sethasPoseLocation_XAxis (std::vector <
				  PoseLocation * >_hasPoseLocation_XAxis)
{
  this->hasPoseLocation_XAxis = _hasPoseLocation_XAxis;
}

void
Vector::get (std::string name)
{
...
Vector::set (std::string name)
{
  ...
  for (unsigned int i = 0; i < hasPartRefAndPose_ZAxis.size (); ++i)
    {
      ss.str ("");
      hasPartRefAndPose_ZAxis[i]->get (hasPartRefAndPose_ZAxis[i]->
				       getname ());
      ss << hasPartRefAndPose_ZAxis[i]->getPartRefAndPoseID ();
      data["hasPartRefAndPose_ZAxis"] =
	data["hasPartRefAndPose_ZAxis"] + " " + ss.str ();
    }
  for (unsigned int i = 0; i < hasPartRefAndPose_XAxis.size (); ++i)
    {
      ss.str ("");
      hasPartRefAndPose_XAxis[i]->get (hasPartRefAndPose_XAxis[i]->
				       getname ());
      ss << hasPartRefAndPose_XAxis[i]->getPartRefAndPoseID ();
      data["hasPartRefAndPose_XAxis"] =
	data["hasPartRefAndPose_XAxis"] + " " + ss.str ();
    }
  for (unsigned int i = 0; i < hasPoseLocation_ZAxis.size (); ++i)
    {
      ...
      data["hasPoseLocation_ZAxis"] =
	data["hasPoseLocation_ZAxis"] + " " + ss.str ();
    }
  for (unsigned int i = 0; i < hasPoseLocation_XAxis.size (); ++i)
    {
      ...
      data["hasPoseLocation_XAxis"] =
	data["hasPoseLocation_XAxis"] + " " + ss.str ();
    }
  dao = new DAO ("Vector");
  dao->set (data);
  delete (dao);
...
void
Vector::copy (std::map < std::string, std::string > object)
{
  ...
  if (this->hasPartRefAndPose_ZAxis.empty ()
      && object["hasPartRefAndPose_ZAxis/PartRefAndPose._NAME"] != "")
    {
      temp =
	Explode (object["hasPartRefAndPose_ZAxis/PartRefAndPose._NAME"], ' ');
  ...
  if (this->hasPartRefAndPose_XAxis.empty ()
      && object["hasPartRefAndPose_XAxis/PartRefAndPose._NAME"] != "")
    {
      temp =
	Explode (object["hasPartRefAndPose_XAxis/PartRefAndPose._NAME"], ' ');
      for (unsigned int i = 0; i < temp.size (); i++)
	{
  ...
  if (this->hasPoseLocation_ZAxis.empty ()
      && object["hasPoseLocation_ZAxis/PoseLocation._NAME"] != "")
    {
      temp =
	Explode (object["hasPoseLocation_ZAxis/PoseLocation._NAME"], ' ');
      for (unsigned int i = 0; i < temp.size (); i++)
	{
	  this->hasPoseLocation_ZAxis.push_back (new PoseLocation (temp[i]));
	}
    }
  if (this->hasPoseLocation_XAxis.empty ()
      && object["hasPoseLocation_XAxis/PoseLocation._NAME"] != "")
    {
      temp =
	Explode (object["hasPoseLocation_XAxis/PoseLocation._NAME"], ' ');
      for (unsigned int i = 0; i < temp.size (); i++)
	{
...
} std::vector < std::string > Vector::Explode (const std::string & str,
					       char separator)
{
