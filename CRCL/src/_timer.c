...
#include "CRCL/_timer_no_config.h"
#include "CRCL/_timer.h"
...
etime ()
{
  ...
  if (clock_gettime (CLOCK_REALTIME, &ts))
    {
      printf ("clock_gettime: errno = %d %s\n", errno,
		       strerror (errno));
    }
...
#define ETIME_RETURNED
  ...
  if (frequency_ok && counter_ok)
    ...
    {
      printf("QueryPerformanceFrequency returned %d, QueryPerformanceCounter returned %d\n",(int)frequency_ok,(int)counter_ok);
    }
  return(0);
#define ETIME_RETURNED
#endif
...
Windows NT */
  ...
  if (TimerCount (&timer_info))
    ...
    {
      printf ("etime: TimerCount returned false.\n");
    }
  return (0);
#define ETIME_RETURNED
