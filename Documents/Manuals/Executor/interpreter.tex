...
The plan is parsed with \class{KittingPlan::parsePlanInstance(const char* filename)} where \class{filename} represents the plan file. The different steps of this function can be described in Figure.
...
\begin{tikzpicture}[node distance=1.5cm, every edge/.style={link}]
  ...
  \node[relationship] (condition) {Condition};
  \node[entity] (openfile) [above=1cm of condition] {Open file};
  \draw[myarrow] (openfile.south) -- (condition.north);
  \node[entity] (loopbody) [below=1cm of condition] {loop};
  \draw[myarrow] (condition.south) -- (loopbody.north);
  
 \draw[myarrow] (loopbody.west) -- ++ (-1,0) -- ++ (0,4)  |-  node [auto] {yes} ($(openfile.south)!.5!(condition.north)$);
 %\draw[myarrow] (loopbody.west)-|($(empty.south)!.5!(condition.north)$);
\node[entity] (closefile) [right=1cm of loopbody] {Close file};
%\draw[myarrow] (condition.east) -- ++ (2.5,0) |- (closefile.north);
\draw[myarrow] (condition.east)-| node [auto] {no} (closefile.north);

\end{tikzpicture}
}
\caption{Process for KittingPlan::parsePlanInstance.}
\label{KittingPlan::parsePlanInstance}
\end{figure}
