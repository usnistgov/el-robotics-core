...
\item \class{Effect} -- An \class{Effect} can consist of \class{Predicate} (\emph{hasEffect\_Predicate}) and \class{Function} (\emph{hasEffect\_Function}). An \class{Effect} can contain o or more \class{FunctionBool} (\emph{hasEffect\_FunctionBool}). An \class{Effect} can contain 0 or more \class{FunctionOperation}. An \class{Effect} belongs to one \class{Action} (\emph{hadByEffect\_Action}). A negative \class{Predicate} is represented with the OWL built-in property assertion \texttt{owl:NegativePropertyAssertion}.
...
As seen in the action failure section, the \process{Predicate Evaluation} process is called by the \process{System Monitor} process to check the truth-value of a predicate. The output of this process is a Boolean value that is redirected back to the \process{System Monitor}. We have represented the concept of ``Spatial Relations'' in the \onto{SOAP} ontology so that any algorithm can compute the truth-value of a predicate.
