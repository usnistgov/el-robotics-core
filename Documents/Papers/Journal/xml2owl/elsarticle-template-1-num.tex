...
sort used in languages such as EXPRESS [\citenum{express}], C++ classes
[\citenum{c++}], and XML Schema Definition Language (XSDL)
[\citenum{xmlSchema0}], [\citenum{xmlSchema1}], [\citenum{xmlSchema2}],
[\citenum{definitive}]. It was decided to use XSDL as the language for
...
[\citenum{albarrak}], [\citenum{bedini}],
[\citenum{yahia}]. Nevertheless, existing software has many
limitations. In some cases, the software converts only XML Schema
[\citenum{tsinaraki}] or requires an existing OWL ontology
[\citenum{rodriguez}]. The majority of tools incorporate information
from either XML schema files or XML instance files, but not both
[\citenum{bohring}], [\citenum{garcia}], [\citenum{ghawi}].  This
...
verification and restructuring of the converted file [\citenum{ghawi}]
or limiting the potential complexity of XML schema files by only
analyzing a single schema at a time [\citenum{garcia}]. Finally, most
tools are implemented using mappings encoded in XML stylesheets
[\citenum{thuy}], [\citenum{yahia}], [\citenum{ontmalizer}], which
seem to scale in exponential time with the length of the converted
document [\citenum{bohring}].  For all of these reasons, a different,
...
[\citenum{lexAndYacc}], [\citenum{flex}] is used to generate the C++ file
domainLex.cc automatically from domain.lex. The Bison YACC processor
[\citenum{yacc}], [\citenum{lexAndYacc}] is used to generate domainYACC.cc
...
for MS Windows [\citenum{msC++}]. As described in Section IV, an additional
...
descriptions may be found for XSDL in [\citenum{xmlSchema0}],
[\citenum{xmlSchema1}], [\citenum{xmlSchema2}], and [\citenum{definitive}],
for XML in [\citenum{xml}], and for OWL in [\citenum{owlPrimer}],
[\citenum{protege}], and [\citenum{owlSpecification}]. XSDL and OWL
...
A number of features of OWL [\citenum{owlPrimer}],
[\citenum{owlSpecification}] and Prot\'eg\'e [\citenum{protege}], the main
...
better OWL consistency checkers is ongoing [\citenum{owlIntegrity}],
[\citenum{moreOwlIntegrity}]. One utility, Pellet, offers some support
for advanced reasoning and debugging [\citenum{pellet}]. In our tests,
