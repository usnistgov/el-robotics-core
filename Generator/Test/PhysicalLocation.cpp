...
void PhysicalLocation::sethasSolidObject_PhysicalLocation(std::vector<SolidObject*> _hasSolidObject_PhysicalLocation){
...
ss.str("");
hasSolidObject_PhysicalLocation[i]->get(hasSolidObject_PhysicalLocation[i]->getname());
ss << hasSolidObject_PhysicalLocation[i]->getSolidObjectID();
data["hasSolidObject_PhysicalLocation"]=data["hasSolidObject_PhysicalLocation"]+" "+ss.str();
}
dao  = new DAO("PhysicalLocation");
dao->set(data);
delete (dao);
...
if(this->hasSolidObject_PhysicalLocation.empty() && object["hasSolidObject_PhysicalLocation/SolidObject._NAME"]!=""){
temp = Explode(object["hasSolidObject_PhysicalLocation/SolidObject._NAME"], ' ' );
for(unsigned int i=0; i<temp.size();i++){
this->hasSolidObject_PhysicalLocation.push_back(new SolidObject(temp[i]));
}
}
