...
\item A set of \class{VacuumEffectorSingleCup} \{\const{part\_gripper},\const{tray\_gripper},\ldots\}: \class{VacuumEffectorSingleCups} are used in a kitting workstation to manipulate \class{Parts}, \class{PartsTrays}, \class{KitTrays}, and \class{Kits}. A \class{VacuumEffectorSingleCup} is attached to a \class{Robot}.
...
\begin{itemize}
\item \stvar{efflocation}: \class{VacuumEffectorSingleCup}$\mathrm{\times S\rightarrow}$\class{Robot} $\cup$ \class{EndEffectorHolder}: designates the location of a \class{VacuumEffectorSingleCup} in the workstation. A \class{VacuumEffectorSingleCup} is either attached to a \class{Robot} or placed in an \class{EndEffectorHolder}.

\item \stvar{r-eff}: \class{Robots}$\mathrm{\times S\rightarrow}$\class{VacuumEffectorSingleCup} $\cup$ \{\textit{nil}\}: designates the \class{VacuumEffectorSingleCup} attached to a \class{Robot} if there is one attached, otherwise \textit{nil}.
...
\item \stvar{efftype}: \class{VacuumEffectorSingleCup}$\mathrm{\times S \rightarrow}$\class{KitTray} $\cup$ \class{Kit} $\cup$ \class{Part}: designates the type of object an \class{VacuumEffectorSingleCup} can hold, i.e., a \class{KitTray}, a \class{Kit}, or a \class{Part}.

\item \stvar{effhold-eff}: \class{EndEffectorHolder}$\mathrm{\times S \rightarrow}$\class{VacuumEffectorSingleCup} $\cup$ \{\textit{nil}\}: designates the object an \class{EndEffectorHolder} is holding, i.e., a \class{VacuumEffectorSingleCup} or nothing.
\end{itemize}
...
\begin{itemize}
 \item \stvar{efflocation}
  \begin{itemize}
  \item \stvar{eff-location}(\class{VacuumEffectorSingleCup},\class{Robot}) ;TRUE iff \class{VacuumEffectorSingleCup} is attached to \class{Robot}
  \item \stvar{eff-location}(\class{VacuumEffectorSingleCup},\class{EndEffectorHolder}) ;TRUE iff \class{VacuumEffectorSingleCup} is in \class{EndEffectorHolder}
  \end{itemize}

 \item \stvar{r-eff}
  \begin{itemize}
  \item \stvar{r-with-eff}(\class{Robot},\class{VacuumEffectorSingleCup}) ;TRUE iff \class{Robot} is equipped with \class{VacuumEffectorSingleCup}
  \item \stvar{r-no-eff}(\class{Robot}) ;TRUE iff \class{Robot} is not equipped with any \class{VacuumEffectorSingleCup}	
  \end{itemize}
 ...
 \item \stvar{efftype}
  \begin{itemize}
    \item \stvar{efftype}(\class{VacuumEffectorSingleCup},\class{KitTray}) ;TRUE iff \class{VacuumEffectorSingleCup} is capable of holding a \class{KitTray}	
    \item \stvar{efftype}(\class{VacuumEffectorSingleCup},\class{Kit}) ;TRUE iff \class{VacuumEffectorSingleCup} is capable of holding a \class{Kit}
    \item \stvar{efftype}(\class{VacuumEffectorSingleCup},\class{Part}) ;TRUE iff \class{VacuumEffectorSingleCup} is capable of holding a \class{Part}
  \end{itemize}

 \item \stvar{effhold-eff}
  \begin{itemize}
    \item \stvar{effhold-eff}(\class{EndEffectorHolder},\class{VacuumEffectorSingleCup}) ;TRUE iff \class{EndEffectorHolder} is holding \class{VacuumEffectorSingleCup}
    \item \stvar{effh-empty}(\class{EndEffectorHolder}) ;TRUE iff \class{EndEffectorHolder} is empty ( not holding a \class{VacuumEffectorSingleCup})
  \end{itemize}
\end{itemize}
...
\begin{enumerate}
\item \op{take-kit-tray}(\const{r},\const{kt},\const{lbwekt},\const{eff},\const{wtable}): The \class{Robot} \const{r} equipped with the \class{VacuumEffectorSingleCup} \const{eff} picks up the \class{KitTray} \const{kt} from the \class{LargeBoxWithEmptyKitTrays} \const{lbwekt}. The \class{WorkTable} \const{wtable} must be \textit{a priori} empty.
...
\item \op{take-part}(\const{r},\const{part},\const{pt},\const{eff},\const{wtable},\const{kit}): The \class{Robot} \const{r} uses the \class{VacuumEffectorSingleCup} \const{eff} to pick up the \class{Part} \const{part} from the \class{PartTray} \const{pt}. The \class{Kit} \const{kit} must \textit{a priori} be on the \const{wtable}.
\begin{center}
\begin{tabular}{ l|l }
  \textit{precond} & \textit{effects} \\
  \hline
...
\item \op{attach-eff}(\const{r},\const{eff},\const{effh}): The \class{Robot} \const{r} attaches the \class{VacuumEffectorSingleCup} \const{eff} which is situated in the \class{EndEffectorHolder} \const{effh}.
\begin{center}
\begin{tabular}{ l|l }
  \textit{precond} & \textit{effects} \\
  \hline
...
\item \op{remove-eff}(\const{r},\const{eff},\const{effh}): The \class{Robot} \const{r} removes the \class{VacuumEffectorSingleCup} \const{eff} and puts it in the \class{EndEffectorHolder} \const{effh}.
\begin{center}
\begin{tabular}{ l|l }
  \textit{precond} & \textit{effects} \\
  \hline
