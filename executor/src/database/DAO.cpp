...
DAO::DAO(std::string name) {
	...
	connection = new Connection("localhost", "root", "zeid", "IPMAS");
...
std::map<std::string, std::string> DAO::get(std::string name) {
	...
	try {
		...
		prep_stmt = connection->getCon()->prepareStatement(
				"SELECT " + className.back() + "ID FROM " + className.back()
						+ " WHERE _Name = ?");
		...
		for (int unsigned i = 0;
				i < DAO::getSqlQueriesDataMulti[className.back()].size(); i++) {
			prep_stmt = connection->getCon()->prepareStatement(
		...
		std::string newTable = "";
		for (int unsigned i = 0;
				i < DAO::getSqlQueriesObjectSingle[className.back()].size();
				i++) {
			try {
				...
				do {
					...
					do {
						res->next();
						res_meta = res->getMetaData();
						for (int j = 0; j < (int) res_meta->getColumnCount();
								++j) {
							if (res->isFirst())
		...
		for (int unsigned m = 0;
				m < DAO::getSqlQueriesObjectMulti[className.back()].size();
				m++) {
			try {
				...
				do {
					...
					// Add Object Single
					for (int unsigned i = 0;
							i < DAO::getSqlQueriesObjectSingle[table].size();
							i++) {
						try {
							stmt = connection->getCon()->createStatement();
