...
This writes a ppm file containing an image that contains the
pictureWindow.

The window is first saved in the tempImage array and then transferred
to the ppmImage array.

The indexing for making ppmImage (the contents of the ppm file) is
...
bottom, and (2) there are 3 chars for each pixel. The critical variables are:
 i is the index for rows of ppmImage. For copying a window from tempImage
   into ppmImage, its value is set initially to the row of ppmImage into
 ...
 k is the index for lines of whatever window is being copied into
   ppmImage. It starts at 0 and is increased by 1 each time around
   the row loop to move upwards in tempImage.
 j is the index for chars on a row.
 start is the char index of place on a line of ppmImage at which to
...
static int windowDump(void) // NO ARGUMENTS
{
  ...
  int height;             // height of dumped image, in lines of pixels
  ...
  height = pictureExtent;
  width = (3 * pictureExtent);
  ...
  for (i=height, k=0; i >= 0; i--, k++)
    { // copy in the picture window
      for (j = 0; j < stop; j++)
	ppmImage[(i * width) + j] = tempImage[(k * stop) + j];
    }
  glutSetWindow(pictureWindow);
  snprintf(fname, fname_size, "commandImage%04d.ppm", counter++);
