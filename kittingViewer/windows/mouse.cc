...
Called by:
  glInit (in viewKitting.cc)
  keyboard (in viewKitting.cc)
...
Called by: motion (in viewKitting.cc)
...
Called By: mouse (in viewKitting.cc)
...
Called by: displayPictureWindow (in viewKitting.cc)

See documentation of msReshape regarding the factor 2 used here.

*/

void msMove(  /* ARGUMENTS                      */
 int isOrtho) /* 1=orthographic, 0=perspective  */
{
  GLfloat orthoZoom;
...
This sets the vector v based on the position of the mouse.
Screen coordinates have Y positive going downward, so the calculation
for v[1] is reversed from the calculation for v[0].

*/

static void msPointToVector( /* ARGUMENTS                      */
 int x,                      /* screen X coordinate in pixels  */
 int y,                      /* screen Y coordinate  in pixels */
 int width,                  /* width (X) of screen in pixels  */
 int height,                 /* height (Y) of screen in pixels */
 float v[3])                 /* vector to set                  */
{
  float d;
  float a;
...
void msReshape( /* ARGUMENTS                       */
 int extent,    /* edge length of screen in pixels */
 int isOrtho)   /* 1=orthographic, 0=perspective   */
{
  GLfloat orthoZoom;
...
If the "rotate" global variable (set by the keyboard in viewKitting.cc)
is non-zero, the left mouse button rotates the picture. Otherwise, the
left mouse button translates the picture. This is to accomodate mice
with only two buttons.

*/

static void msStartMotion( /* ARGUMENTS                               */
 int x,                    /* screen X coordinate in pixels           */
 int y,                    /* screen Y coordinate in pixels           */
 int button)               /* indicates left, right, or middle button */
{
...
static void msStopMotion() /* NO ARGUMENTS */
{
