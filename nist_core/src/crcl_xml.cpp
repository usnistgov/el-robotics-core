...
namespace Xml
{
  LIBRARY_API CrclXml::CrclXml (CrclXmlParams *params) :
    params_(params)
  {
    xaxisactive = xaxisactive = false;
  }
...
*/
      ...
      if (strcmp (tagName.c_str(), "CRCLCommand") == 0)
      {
        for (; nameiter != attr.name.end(); ++nameiter, ++valiter)
        {
          if (strcmp (nameiter->c_str(), "xsi:type") == 0)
          {
            ...
            else if (strcmp (valiter->c_str(), "SetRelativeSpeedType") == 0)
            {
  ...
  */
  ...
  LIBRARY_API void CrclXml::exception (char *where, const char *what)
  {
    static char message[1024];
    sprintf (message, "Exception in \" %s \" : %s", where, what);
    cout << message << endl;
    //! JAM:  TODO:  What to do with message now that it's created?
  }
