...
#if 1
    current.position.x = 0; // .x = 0;
    current.position.y = 0; // .y = 0;
    current.position.z = 0; // .z = 0;
    current.rotation.x = 0; // .xrot = 0;
    current.rotation.y = 0; // .yrot = 0;
    current.rotation.z = 0; // .zrot = 0;
#endif
}
...
std::vector<urdf::Pose> TrajectoryMaker::GetPosesPlan() {
    return plannedposes;
}
...
bool TrajectoryMaker::Plan(urdf::Pose & curpose, urdf::Pose & goalpose) {
    plannedposes = makeCartesianTrajectory(currates, curpose, goalpose);
    return true;
}

bool TrajectoryMaker::Plan(std::vector<urdf::Pose>& waypoints) {
    assert(0);
    return true;
}
 ...
 */
void TrajectoryMaker::setCurrent(urdf::Pose poseIn) {
    current = poseIn;
}
...
std::vector<urdf::Pose> TrajectoryMaker::makeCartesianTrajectory(double final_velocity,
...
std::vector<urdf::Pose> TrajectoryMaker::makeCartesianTrajectory(IRate rates,
        urdf::Pose _curPos,
        urdf::Pose _goalPos) {
    ...
    while (rates.MsFlag() != RCS::MS_IS_DONE) {
        ...
        urdf::Pose pose = computeTranslation(_curPos, _goalPos, totalIncrements);
        poses.push_back(pose);
        current = urdf::Vector3(pose.position.x, pose.position.y, pose.position.z);
    ...
    for (size_t i = 0; i < poses.size(); i++) {
        ...
        urdf::Rotation q = slerp(_curPos.rotation, _goalPos.rotation, t);
...
urdf::Pose TrajectoryMaker::computeTranslation(urdf::Pose & _curPos, urdf::Pose & _goalPos, double dIncrement) {
    urdf::Vector3 current(_curPos.position.x, _curPos.position.y, _curPos.position.z);
    urdf::Vector3 goal(_goalPos.position.x, _goalPos.position.y, _goalPos.position.z);
