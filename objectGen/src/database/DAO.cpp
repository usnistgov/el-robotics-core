...
//		std::cout<<results["_NAME"][i] << std::endl;
//	}
	//Part p("part_c_1");
	//p.get("part_c_1");
	//std::cout << p.gethasPart_SerialNumber() << std::endl;
//	return 0;

//}
DAO::DAO(std::string name) {
	this->className.push_back(name);
	connection = new Connection("localhost", "root", "warnerBr0s", "OWL");
...
void DAO::fillGetSqlQueries() {
	...
	DAO::getSqlQueriesDataSingle["Robot"]
			= "SELECT Robot.RobotID AS 'Robot.RobotID', Robot._NAME AS 'Robot._NAME', Robot.hasRobot_Description AS 'Robot.hasRobot_Description', Robot.hasRobot_MaximumLoadWeight AS 'Robot.hasRobot_MaximumLoadWeight' FROM Robot, SolidObject WHERE  Robot.RobotID = ? AND SolidObject.SolidObjectID = Robot.RobotID";
	DAO::getSqlQueriesDataSingle["Kit"]
			= "SELECT Kit.KitID AS 'Kit.KitID', Kit._NAME AS 'Kit._NAME', Kit.isKit_Finished AS 'Kit.isKit_Finished' FROM Kit, SolidObject WHERE  Kit.KitID = ? AND SolidObject.SolidObjectID = Kit.KitID";
	...
	DAO::getSqlQueriesDataSingle["PartRefAndPose"]
			= "SELECT PartRefAndPose.PartRefAndPoseID AS 'PartRefAndPose.PartRefAndPoseID', PartRefAndPose._NAME AS 'PartRefAndPose._NAME' FROM PartRefAndPose, DataThing WHERE  PartRefAndPose.PartRefAndPoseID = ? AND DataThing.DataThingID = PartRefAndPose.PartRefAndPoseID";
	DAO::getSqlQueriesDataSingle["BoxyShape"]
			= "SELECT BoxyShape.BoxyShapeID AS 'BoxyShape.BoxyShapeID', BoxyShape._NAME AS 'BoxyShape._NAME', BoxyShape.hasBoxyShape_Width AS 'BoxyShape.hasBoxyShape_Width', BoxyShape.hasBoxyShape_Top AS 'BoxyShape.hasBoxyShape_Top', BoxyShape.hasBoxyShape_Height AS 'BoxyShape.hasBoxyShape_Height', BoxyShape.hasBoxyShape_Length AS 'BoxyShape.hasBoxyShape_Length', ShapeDesign.hasShapeDesign_Description AS 'ShapeDesign.hasShapeDesign_Description' FROM BoxyShape, ShapeDesign, DataThing WHERE  BoxyShape.BoxyShapeID = ? AND ShapeDesign.ShapeDesignID = BoxyShape.BoxyShapeID AND DataThing.DataThingID = BoxyShape.BoxyShapeID";
	DAO::getSqlQueriesDataSingle["Part"]
			= "SELECT Part.PartID AS 'Part.PartID', Part._NAME AS 'Part._NAME', Part.hasPart_SerialNumber AS 'Part.hasPart_SerialNumber' FROM Part, SolidObject WHERE  Part.PartID = ? AND SolidObject.SolidObjectID = Part.PartID";
	...
	DAO::getSqlQueriesDataSingle["PartsTray"]
			= "SELECT PartsTray.PartsTrayID AS 'PartsTray.PartsTrayID', PartsTray._NAME AS 'PartsTray._NAME', PartsTray.hasPartsTray_SerialNumber AS 'PartsTray.hasPartsTray_SerialNumber' FROM PartsTray, SolidObject WHERE  PartsTray.PartsTrayID = ? AND SolidObject.SolidObjectID = PartsTray.PartsTrayID";
	...
	DAO::getSqlQueriesDataSingle["KitTray"]
			= "SELECT KitTray.KitTrayID AS 'KitTray.KitTrayID', KitTray._NAME AS 'KitTray._NAME', KitTray.hasKitTray_SerialNumber AS 'KitTray.hasKitTray_SerialNumber' FROM KitTray, SolidObject WHERE  KitTray.KitTrayID = ? AND SolidObject.SolidObjectID = KitTray.KitTrayID";
	...
	DAO::getSqlQueriesDataSingle["KitDesign"]
			= "SELECT KitDesign.KitDesignID AS 'KitDesign.KitDesignID', KitDesign._NAME AS 'KitDesign._NAME' FROM KitDesign, DataThing WHERE  KitDesign.KitDesignID = ? AND DataThing.DataThingID = KitDesign.KitDesignID";
	...
	DAO::getSqlQueriesDataSingle["LargeContainer"]
			= "SELECT LargeContainer.LargeContainerID AS 'LargeContainer.LargeContainerID', LargeContainer._NAME AS 'LargeContainer._NAME', LargeContainer.hasLargeContainer_SerialNumber AS 'LargeContainer.hasLargeContainer_SerialNumber' FROM LargeContainer, SolidObject WHERE  LargeContainer.LargeContainerID = ? AND SolidObject.SolidObjectID = LargeContainer.LargeContainerID";
	temp.clear();
	...
	temp.clear();
	temp.push_back("StockKeepingUnit/hasPartRefAndPose_Sku");
	...
	temp.clear();
	temp.push_back("StockKeepingUnit/hasPartsBin_Sku");
	...
	temp.clear();
	temp.push_back("StockKeepingUnit/hasPartsTray_Sku");
	...
	temp.clear();
	temp.push_back("-Kit/hasKit_Design");
	...
	temp.clear();
	temp.push_back("EndEffector/hasSku_EndEffectors");
	...
	temp.clear();
	temp.push_back("StockKeepingUnit/hasSku_EndEffectors");
...
std::map<std::string, std::vector<std::string> > DAO::getAll(std::vector<
	...
	try {
		...
		while (res->next()) {
			for (int i = 0; i < (int) res_meta -> getColumnCount(); ++i) {
				std::vector<std::string> temp = result[res_meta -> getColumnLabel(i
						+ 1)];
