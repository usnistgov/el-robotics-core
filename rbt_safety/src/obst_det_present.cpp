...
//Decrease Rmin to detect higher objects (must be greater than -8)
//Increase Rmax to detect lower objects (must be less than 8)
#define set_Rmin 0.1 //Usually set at just below the ceiling
#define set_Rmax 1.1 //Usually set at just above the floor
...
const float Rmax = set_Rmax + 1.9;
const float Rmin = set_Rmin + 2.1;
...
int filt_PC = set_filt_PC/15;//100
int gxrst, gzrst;
double start_time = 0;
double start_exec = 0;
...
void incomingCloudCallback(const sensor_msgs::PointCloud2ConstPtr& cloud)
{
  ...
  { 
	    ...
	    float z = *reinterpret_cast<const float*>(ptr + zoff);
 	    ...
 	    if ((((x/x_sep)+(x_res/2)) > 0) && (((x/x_sep)+(x_res/2)) < (x_res)))
	      ...
	      PCount[x_ind][z_ind] = PCount[x_ind][z_ind] + 1;
            ...
            if ((y < (maxh_mask[x_ind][z_ind] - h_tolerance)) && ((y+2) > Rmin) && ((y+2) < Rmax))
              HOCount[x_ind][z_ind] = HOCount[x_ind][z_ind] + 1;

            //Record only the highest point value in each bin
            if ((y < g_PlotArray[x_ind][z_ind]) && ((y+2) > Rmin) && ((y+2) < Rmax))
            ...
            {
              //If the point is in a STOP zone
              if ((maskimg(x_ind, z_res-z_ind, 0, 0) == red[0]) && 
              (maskimg(x_ind, z_res-z_ind, 0, 1) == red[1]) && (maskimg(x_ind, z_res-z_ind, 0, 2) == red[2]))
              {  
                //Check if any of the points around it (top, left, and diagonal) are also above the minimum height threshold
      ...
      if ((t - warn_end_time) >= warn_delay)
      {
        if (stop_warn)
        {
          ROS_WARN("Obstruction: STOP\n");
          stop_warn = false;
        }
        else if (warn_warn)
        {
          ROS_WARN("Obstruction: WARN\n");
          warn_warn = false;
        }
      ...
      if (init_samp_cnt == init_samp_cnt_max)
      {
        ...
        for (Wx = 0; Wx <= x_res; Wx++)
        {
          for(Wz = 0; Wz <= z_res; Wz++)
          {
            ...
            {
              if (stoplist[Wx][Wz] == true)
	      {
                color[0] = redfake[0];
                color[1] = redfake[1];
                color[2] = redfake[2];
              }
              else if (warnlist[Wx][Wz] == true)
              {
...
int main(int argc, char **argv)
{
  ...
  ROS_INFO("Please wait for initialization (60s)...");
  start_time = ros::Time::now().toSec();
  while(start_exec < (start_time + 60))
  {    
    start_exec = ros::Time::now().toSec();
  }
  ROS_INFO("Generating mask image...");
