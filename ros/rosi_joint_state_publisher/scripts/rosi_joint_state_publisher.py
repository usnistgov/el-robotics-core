...
# Use:
# rosi_joint_state_publisher use_gui:=true
import rospy
import random
...
class JointStatePublisher():
    def __init__(self):
        ...
        pub_def_positions = get_param("publish_default_positions", False)
	...
	#self.source_cb(msg)
        ...
        if use_gui:
         ...
         # Mods by John MIchaloski NIST to work with ROS-I
        source_list = get_param("source_list_in", ["joint_states"])
        self.sources = []
#        for source in source_list:
#            self.sources.append(rospy.Subscriber(source, JointState, self.source_cb))

	source_list_out="joint_path_command"
   	if rospy.has_param('source_list_out'):
               source_list_out = get_param("source_list_out", [])[0]
           
   	#self.pub = rospy.Publisher(source_list_out, JointState, queue_size=5)
   	#msg = rospy.wait_for_message("joint_states", JointState)
   	#self.source_cb(msg)
	self.pub = rospy.Publisher(source_list_out, JointTrajectory, queue_size=5)
    ...
    def joint_update(self):
        ...
        for i in range(len(msg.name)):
            ...
            if position is not None:
                ...
                print "joint[", name, "].position=", position
...
#        self.gui.sliderUpdateTrigger.emit()
	...
	lastmsg= JointTrajectory()
	lastmsg.points.append(JointTrajectoryPoint())
	    ...
	    lastmsg.header.stamp = rospy.Time.now()	
            ...
            if has_position:
                msg.points[0].positions = num_joints * [0.0]
            if has_velocity:
                msg.points[0].velocities = num_joints * [0.0]
            if has_effort:
                msg.points[0].effort = num_joints * [0.0]
            ...
            for i, name in enumerate(self.joint_list):
                ...
                if has_position and 'position' in joint:
                    msg.points[0].positions[i] = joint['position'] * factor + offset
                if has_velocity and 'velocity' in joint:
                    msg.points[0].velocities[i] = joint['velocity'] * factor
                if has_effort and 'effort' in joint:
                    msg.points[0].effort[i] = joint['effort']
	    if(lastmsg.points[0].positions != msg.points[0].positions):
...
class JointStatePublisherGui(QWidget):
    ...
    def __init__(self, title, jsp, num_rows=0):
        print "JointStatePublisherGui __init__"
        ...
        for name in self.jsp.joint_list:
            ...
            self.sliders.append(joint_layout)
        ...
        if self.num_rows == 0:
            self.num_rows = len(self.sliders)  # equals VBoxLayout
        # Generate positions in grid and place sliders there
        self.positions = self.generate_grid_positions(len(self.sliders), self.num_rows)
        for item, pos in zip(self.sliders, self.positions):
        ...
        self.maxrowsupdown.setMaximum(len(self.sliders))
    ...
    def update_sliders(self):
        for name, joint_info in self.joint_map.items():
            joint = joint_info['joint']
            print "gui joint[", name, "].position=", joint['position']
...
if __name__ == '__main__':
    try:
        print "ros init"
        rospy.init_node('rosi_joint_state_publisher')
