...
3. XXXYACC.o (from XXX.y, which is generated by GenXMiller)
4. XXXLex.o (from XXX.lex, which is generated by GenXMiller)

One line in each of XXX.y and XXX.lex needs to be changed manually from
 ...
 XmlSchema * schema)                 /* schema to build classes for */
{
  ...
  if ((topElement = dynamic_cast<XmlElementRefable *>(*iter)))
    ...
    {
      fprintf(stderr, "first item in schema contents must be root element\n");
      exit(1);
    }
  for (iter++ ; iter != schema->contents2->end(); iter++)
    {
...
Called By:
 ...
 xml2owlGenerator::printOwlElementLocalIDREF
...
Called By:
  ...
  xml2owlGenerator::printHeaderSchemaClassesComplex
...
FIX - Add XmlToken here and elsewhere stringy types are allowed.
 ...
 XmlComplexType * complx)                /* XmlComplexType to print code for */
{
  ...
  if ((other = dynamic_cast<XmlOtherContent *>(complx->item)))
    {
      if ((sequence = dynamic_cast<XmlSequence *>(other->base)))
	printCodeSequence(sequence, complx->newName, complx->abstract);
 ...
 char * newName)                  /* newName of complx                      */
{
  ...
  //enterKid(baseType, complx); redundant to enterKids -- delete eventually
  ...
  if (!complx->abstract)
      ...
      if (hasSequence)
	{
	  ...
	  fprintf(ccFile, "  printer.startIndi(Name, \"%s\", outFile);\n",
		  noTypeName);
	  printCodePrintParentItems(baseType);
	  if (items)
	    printCodePrintItems(items, noTypeName);
	  fprintf(ccFile, "  printer.endIndi(\"%s\", outFile);\n", noTypeName);
	}
...
void xml2owlGenerator::printCodeEnd()
{
  char * topName;
  ...
  fprintf(ccFile, "{\n");
  fprintf(ccFile, "  printer.printHeader(outFile);\n");
  fprintf(ccFile, "}\n");
 ...
 char * haserName)                         /* deTyped name of element owner */
{
  ...
  if ((element->maxOccurs == -1) || (element->maxOccurs > 1))
    { // multiple elements are allowed, need list, (-1 means unbounded)
      printCodePrintElementMulti(element, haserName, simple, isBasic);
    }
  else if ((element->maxOccurs == 1) || (element->maxOccurs < -1))
    { // only one element allowed (maxOccurs < -1 means maxOccurs not used)
      printCodePrintElementSingle(element, haserName, simple, isBasic);
    }
}
  ...
  if (simple)
    {
      ...
      fprintf(ccFile, "         iter != %s->end(); iter++)\n",
	      element->newName);
      fprintf(ccFile, "      {\n");
      fprintf(ccFile, "        (*iter)->printProp(\"has%s_%s\",\n",
	      haserName, element->name);
    ...
    { // isBasic is true in this case
      ...
      fprintf(ccFile, "         iter != %s->end(); iter++)\n",
	      element->newName);
      fprintf(ccFile, "      {\n");
      fprintf(ccFile, "        printer.printObjRefProp(\"has%s_%s\",\n",
	      haserName, nameName);
      fprintf(ccFile,
	      "                                Name, *iter, outFile);\n");
      fprintf(ccFile, "      }\n");
  ...
  else if (isBasic)
    {
      ...
      fprintf(ccFile,  "        iter != %s->end(); iter++)\n",
	      element->newName);
      fprintf(ccFile, "      {\n");
      fprintf(ccFile, "        printer.print%sProp(\"has%s_%s\",\n",
	      typeName, haserName, element->name);
 ...
 bool isBasic)                             /* true if basic, else false     */
{
  ...
  if (simple)
    {
      fprintf(ccFile, "%s%s->printProp(\"has%s_%s\",\n",
	      space, element->newName, haserName, element->name);
    ...
    { // isBasic is true in this case
      deNameName(element->newName, nameName);
      fprintf(ccFile, "%sprinter.printObjRefProp(\"has%s_%s\",\n",
	      space, haserName, nameName);
    ...
    {
      findCppTypeName(element->newTyp, typeName);
      fprintf(ccFile, "%sprinter.print%sProp(\"has%s_%s\",\n",
	      space, typeName, haserName, element->newName);
    ...
    {
      fprintf(ccFile, "%sprinter.printObjProp(\"has%s_%s\",\n",
	      space, haserName, element->name);
...
void xml2owlGenerator::printCodePrintIndividuals()
{
  std::list<XmlType *>::iterator iter;
  XmlComplexType * complx;
  ...
  for (iter = classes.begin(); iter != classes.end(); iter++)
	  ...
	  (!(complx->abstract)))
	{
	  ...
	  fprintf(ccFile, "  if (%s::individuals.size() > 1)\n",
		  complx->newName);
	  fprintf(ccFile, "    printer.printIndividuals(\"%s\",\n",
		  nameBuffer);
 ...
 char * haserName)                          /* deTyped name of owner type */
{
  ...
  for (iter = items->begin(); iter != items->end(); iter++)
	...
	printCodePrintElement(element, haserName);
    }
}
...
derived from XMLSCHEMAINSTANCEBASE and must be an abstract type that
has a Name element.
...
If the complex type that contains the sequence is not abstract (as indicated
by the "abstract" argument being true), this also prints a PrintOwl function.
 ...
 char * baseName)                      /* base name to use */
{
  ...
  fprintf(ccFile, "  header->printOwl(outFile);\n");
...
FIX - maybe. Replace literal "yy" with yyprefix.
...
Called By: xml2owlGenerator::printHeaderSchemaClassesComplex
...
printSelf function causes a compiler error). If also prints a virtual
  ...
  while (progress)
      ...
      for (iter = classes.begin(); iter != classes.end(); iter++)
	{
	  if ((complx = dynamic_cast<XmlComplexType *>(*iter)))
		...
		continue;
	      printHeaderSchemaClassesComplex(complx, &totalPrinted, &progress);
	    }
	  else if ((simple = dynamic_cast<XmlSimpleType *>(*iter)))
	    {
	      if (simple->hhPrinted == true)
		continue;
	      printHeaderSchemaClassesSimple(simple, &totalPrinted, &progress);
	    }
	  else
	    {
...
void xml2owlGenerator::printHeaderSchemaClassesComplex(/* ARGUMENTS         */
 XmlComplexType * complx,              /* XmlComplexType to print class for */
...
/* xml2owlGenerator::printHeaderSchemaClassesSimple
...
void xml2owlGenerator::printHeaderSchemaClassesSimple(/* ARGUMENTS         */
...
Called By: xml2owlGenerator::printHeaderSchemaClassesSimple
 ...
 char * argv[]) /* array of executable name and command arguments */
{
  ...
  generator.buildClasses(xmlSchemaFile->schema);
  generator.enterKids();
  generator.checkClasses();
  argv[2][0] = toupper(argv[2][0]);
  generator.printDriver(argv[2], generator.topElement->newName);
  generator.printHeader(argv[2]);
  generator.printCode(argv[2]);
  return 0;
}
