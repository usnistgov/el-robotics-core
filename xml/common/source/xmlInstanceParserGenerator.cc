...
position (for example, a comment, a function declaration, an attribute
...
attribute names. Within a single C++ class, no XML element name is used
for more than one attribute name, so duplicate XML element names are
not a problem.
...
Optional elements (those with minOccurs="0") that may occur at most once
are handled in YACC by allowing the optional to be empty and returning a
null pointer in that case. In the C++ model of a class, since all C++
attributes are pointers, nothing special needs to be done to handle an
element that occurs 0 or 1 times. If it does not occur, a null pointer
is usedl
...
C++ attribute representing that element is XXX.

The XXX class has the pairs attribute defined as a std::list because
...
1c. by putting in one or more references to a group of attribute definitions.
   ...
   XmlAttributeGroup. A group of definitions that may be referenced is called
   an XmlAttributeGroupRefable and may occur only at the top level of
...
XmlAttributeLoner with a definition, and any XmlAttributeGroup is
...
respectively). The value of the data is stored in a "val" attribute.
A "bad" attribute indicates whether the data violates the restrictions
...
function and sets the value of "bad". The the YACC parser checks the value
...
1. Any chain of C++ class derivations representing a chain of XML
   ...
   Each class in the chain inherits two C++ attributes from the C++ root
   ...
   supposed to follow. The class has no other C++ attributes.
...
3. The constructor that takes a char * argument calls the constructor for
   ...
   "bad" is false (meaning everthing is OK so far), the child constructor
...
2. The next lext level up has one or more restrictions, so it has a "bad"
data member and a checking function that checks the restrictions of this
...
inherits the "bad" data member from the next level down. The constructor at
...
2. A constructor taking an argument for each data member. The constructor
   sets each data member to the corresponding value given in the arguments.
3. A constructor taking an argument for each data member not corresponding
   to an attribute. These constructors are used in YACC code.
4. A destructor taking no arguments that does nothing. It may be useful
   to change this so that the destructor cleans up better.
5. A printSelf function that prints the item in an XML file. This takes
   one argument that is a FILE pointer.
6. A data member for each attribute and element. In the case of XmlChoice,
   more items are included as described above.

For classes representing XML restrictions, there is also a boolean "bad"
data member and a checkVal or xxxIsBad function called by the constructor
...
#include "xmlInstanceParserGenerator.hh"
...
5. If the item is an XmlAttributeGroup, it is entered into the
...
XmlElementLocal.  To deal with this, the generator has an attribute named
...
below) that represents a rule that givess the type and its subtypes
as alternatives.
...
to attributes. The C++ attributes representing XML attributes are set
when the badAttributes function runs (which happens when the badAttriubutes
...
This prints in the text for the YACC file the items in the sequence of
an extension.
...
a "bad" attribute that will need checking after the constructor is called.
...
sets the "bad" attribute of the class instance to true. After the
constructor runs, the rule checks whether the "bad" attribute is true
...
To cover all possible cases, there are probably more things to do when
needsList is true than are currently being done. What is here handles
everything in the existing test suite.
...
element or attribute is being processedl
...
This makes the attributeLonerRefables std::list, derived from the contents2
of the schema.
...
This enters the lone in the lonerList in alphabetical order by name.
...
Returned Value: XmlElementRefable *
  This looks through the elementRefables and returns the one with the
...
This expects the buffer to contain a string such as
  ...
  fprintf(outFile, "<UnitLength>%f</UnitLength>\n", *UnitLength);
...
An XmlAttributor may be an XmlAttributeLoner or an XmlAttributeGroup.
...
decl has a ref, the referenced XmlAttributeLonerRefable, lonerRefabile,
...
If the XmlAttributor is an XmlAttributeGroup, it may only have a ref.
In this case, the referenced XmlAttributeGroupRefable, groupRefable,
is found in the groupRefables. If the groupRefable already has
 ...
 std::list<XmlAttributor *> * old)      /* attribute list to take from */
{
  ...
  for (iter = old->begin(); iter != old->end(); iter++)
      ...
      if ((decl = dynamic_cast<XmlAttributeLoner *>(*iter)))
	{
      ...
      else if ((group = dynamic_cast<XmlAttributeGroup *>(*iter)))
	{
	  otor = attributeGroupRefables->find(group->ref);
...
This:
1. checks the arguments. There must be 2, 4, or 6 command arguments,
   so that argc must be 3, 5, or 7. If not, this calls usageMessage,
   which explains how to call the generator then exits.
   If the number of arguments is OK, this checks that there is at most
   one of each of the following argument pairs:
     -p <prefix>
...
makes some checks and then sets the value of the C++ attribute
representing the XML attribute with the name given in the
AttributePair. The checks are:
...
void generator::printCppCodeBadAttributes(
 char * className,
 std::list<XmlAttributeLoner *> * allAttributes)
{
  std::list<XmlAttributeLoner *>::iterator iter;
...
"      decl = *iter;\n");
  for (iter = allAttributes->begin(); iter != allAttributes->end(); iter++)
	...
	strcpy(attClassName, decl->newTyp);
      ...
      fprintf(ccFile, "if (strncmp(decl->name, \"%s\", %d) == 0)\n",
	      decl->name, NAMESIZE);
      needsElse = 1;
      fprintf(ccFile, "        {\n");
...
void LocalUnitType::printSelf(FILE * outFile)
{
  if (LocalUnitTypeType == UserDefinedUnitsE)
    {
      doSpaces(0, outFile);
      fprintf(outFile, "<UserDefinedUnits>");
      fprintf(outFile, "%f", *(LocalUnitTypeValue.UserDefinedUnits));
      fprintf(outFile, "</UserDefinedUnits>\n");
    }
  else if (LocalUnitTypeType == TimeE)
    {
      ...
      fprintf(outFile, "%f", *(LocalUnitTypeValue.Time));
      fprintf(outFile, "</Time>\n");
    }
...
}
      ...
      if ((element = dynamic_cast<XmlElementLocal *>(*iter)))
	{
	  ...
	  if (element->needList)
	      ...
	      if (simple || isBasic)
		{
		  fprintf(ccFile, "      fprintf(outFile, \"<%s>\");\n",
			  element->newName);
		  fprintf(ccFile, "      %sValue.%s->printSelf(outFile);\n",
			  newName, element->newName);
		}
	      else if (complx)
		{
 ...
 std::list<XmlAttributeLoner *> * allAttributes) /* all attributes of class */
{
  ...
  fprintf(ccFile, "%s::%s() {", newName, newName);
  if (hasAttribs)
    { // base constructor not called, so set all attributes to 0
      fprintf(ccFile, "\n");
      printCppCodeAttributeSettings0(allAttributes);
    }
  fprintf(ccFile, "}\n");
  // finished printing constructor with no arguments
  fprintf(ccFile, "\n");
  if (hasSequence)
    {
      ...
      if (sequence)
	{
      ...
      fprintf(ccFile, ")\n");
      fprintf(ccFile, "{\n");
      if (hasAttribs) // base constructor called so set only new attibutes to 0
	printCppCodeAttributeSettings0(extend->newAttribs);
      if (items)
	printCppCodeSequenceSettings(items);
      fprintf(ccFile, "  printTypp = false;\n");
      fprintf(ccFile, "}\n");
      // finished printing constructor with arguments for sequence items only
      fprintf(ccFile, "\n");
    }
  if (hasAttribs)
    {
	...
	printCppCodeSequenceSettings(items);
      fprintf(ccFile, "  printTypp = false;\n");
      fprintf(ccFile, "}\n");
      // finished printing constructor with arguments for all items
      fprintf(ccFile, "\n");
    }
  fprintf(ccFile, "%s::~%s() {}\n", newName, newName);
  fprintf(ccFile, "\n");
  ...
  fprintf(ccFile, "}\n");
  //if (hasAttribs)
  //  printCppCodeBadAttributes(extend->newAttribs, newName, extend->base);
}
...
void generator::printCppCodeEnd()
{
  ...
  fprintf(ccFile, "XmlHeaderFor%s::~XmlHeaderFor%s() {}\n",
	  classBaseName, classBaseName);
  fprintf(ccFile, "\n");
  fprintf(ccFile, "void XmlHeaderFor%s::printSelf(\n", classBaseName);
...
the following segment of code. The segment of code prints one line.
  ...
  doSpaces(0, outFile);
  fprintf(outFile, "<TemperatureUnits>");
  TemperatureUnits->printSelf(outFile);
  fprintf(outFile, "</TemperatureUnits>\n");
...
prints at least three lines.
  ...
  if (AngularUnits)
    {
      doSpaces(0, outFile);
      fprintf(outFile, "<AngularUnits");
      AngularUnits->printSelf(outFile);
      doSpaces(0, outFile);
      fprintf(outFile, "</AngularUnits>\n");
    }
...
that doSpaces is not called a second time.
  ...
  if (LinearUnits)
    {
      doSpaces(0, outFile);
      fprintf(outFile, "<LinearUnits");
      LinearUnits->printSelf(outFile);
      fprintf(outFile, "</LinearUnits>\n");
    }
...
The restrictions on the sList are checked when the sList is read and
when the sList is printed. The way the YACC works, the constructor
gets a completed std::list that has been made in the process of
building the std::list as if it were unrestricted.
...
an XmlComplexType containing a sequence.
...
This prints the C++ code for an sList
...
void generator::printCppCodeStart() /*  NO ARGUMENTS  */
{
  ...
  fprintf(ccFile, "#include \"%sClasses.hh\"\n", baseNameNoPath);
  ...
  fprintf(ccFile, "%sFile::~%sFile() {}\n", classBaseName, classBaseName);
  fprintf(ccFile, "\n");
  fprintf(ccFile,"void %sFile::printSelf(FILE * outFile)\n", classBaseName);
  fprintf(ccFile, "{\n");
  fprintf(ccFile, "  version->printSelf(outFile);\n");
  fprintf(ccFile, "  fprintf(outFile, \"<%s\\n\");\n", top->name);
  fprintf(ccFile, "  header->printSelf(outFile);\n");
  fprintf(ccFile, "  %s->printSelf(outFile);\n",  top->newName);
  fprintf(ccFile, "  fprintf(outFile, \"</%s>\\n\");\n",  top->name);
  fprintf(ccFile, "}\n");
  printStarLine(ccFile, true, true);
...
printed, and
(2) the C++ class attributes representing the XML attributes.
...
This prints the C++ attributes of a class resulting from the parent type of
an XmlSimpleContentExtension (and its parent type, etc.). The first item
printed is the simple type at the end of the type chain. This calls itself
recursively to get all the way through the chain.
...
attribute, however, since the badAttributes function of any type that
...
void generator::printCppHeaderStart() /* NO ARGUMENTS */
{
  ...
  for (iter = includedSchemas->begin(); iter != includedSchemas->end(); iter++)
    {
      fprintf(hhFile, "#include \"%sClasses.hh\"\n", *iter);
    }
  if (moreIncludes)
    {
...
1. read a target namespace declaration if there is a target namespace.
   ...
   where <targetNamespace> is the targetNamespace specifed in the schema.
...
void generator::printLexEnd() /* NO ARGUMENTS  */
{
  ...
  fprintf(lexFile, "\n");
  fprintf(lexFile, "\\\"[^\\\"]+\\\"                 {ECH;\n");
  fprintf(lexFile, "                            int first;\n");
  fprintf(lexFile, "                            int n;\n");
  fprintf(lexFile, "                            for (first = 0; "
	  "%stext[first] != '\"'; first++);\n", yyprefix);
  fprintf(lexFile, "                            first++;\n");
  fprintf(lexFile, "                            for (n = first; ");
  fprintf(lexFile, "%stext[n] != '\"'; n++);\n", yyprefix);
  fprintf(lexFile, "                            %stext[n] = 0;\n", yyprefix);
  fprintf(lexFile, 
	  "                            %slval.sVal = strdup(%stext + first);\n",
	  yyprefix, yyprefix);
...
1. The extensionInfos attribute of the xmlInstanceParserGenerator.
2. The extensions attribute of the XmlComplexType class. This std::list
...
{
  fprintf(stderr, "usage: %s [-h <header>] [-p <prefix>] -x <schema>\n",
	  command);
