...
extern CirclesAndSpheresFile * CirclesAndSpheresTree;
extern FILE * yyin;
extern int yyparse();
 ...
 char * argv[]) /* array of executable name and command arguments */
{
  ...
  fprintf(outFile, "Prefix(geo:=<%s/geometrySchema3Classes.owl#>)\n", argv[3]);
  fprintf(outFile, "Prefix(cir:=<%s/CircleAndSphereClasses.owl#>)\n", argv[3]);
  fprintf(outFile, "Prefix(poi:=<%s/PointAndVectorClasses.owl#>)\n", argv[3]);
  fprintf(outFile, "Ontology(<%s/%s>\n", argv[3], argv[2]);
  fprintf(outFile, "Import(<file:geometrySchema3Classes.owl>)\n");
  CirclesAndSpheresTree->printOwl(outFile);
  fclose(outFile);
  return 0;
}
