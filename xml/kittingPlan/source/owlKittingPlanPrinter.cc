...
extern KittingPlanFile * KittingPlanTree;
extern FILE * yyin;
extern int yyparse();
 ...
 char * argv[]) /* array of executable name and command arguments */
{
  ...
  fprintf(outFile, "Prefix(ktp:=<%s/kittingPlanClasses.owl#>)\n", argv[3]);
  fprintf(outFile, "Prefix(ktw:=<%s/kittingWorkstationClasses.owl#>)\n", argv[3]);
  fprintf(outFile, "Ontology(<%s/%s>\n", argv[3], argv[2]);
  fprintf(outFile, "Import(<file:kittingPlanClasses.owl>)\n");
  KittingPlanTree->printOwl(outFile);
  fclose(outFile);
  return 0;
}
